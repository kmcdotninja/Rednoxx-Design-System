...
| Consultation | `Encounter`, `Condition`, `ClinicalImpression`, `DocumentReference` | diagnosis typeahead writes Condition (ICD-10); note sections → ClinicalImpression / DocumentReference — confirm the actual note-resource choice with the backend contract before wiring the form; sign-off → Encounter status + audit |
